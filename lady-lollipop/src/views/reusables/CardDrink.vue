    ...
    <ul class="row-div">
      <li class="li-cards">
        ...
        <div>
          ...
          <p class="price-p" step="0.01">{{ drink.price }}</p>
          <p class="stock-p">{{ drink.stock }} in stock</p>
        </div>
        <div>
        ...
        <div class="index-button-div">
          ...
          <el-dialog top="13vh" width="43%" :visible.sync="editFormVisible">
            <EditDrink :drinks="drink" v-on:changeDisplay="editFormNotVisible($event)" />
          </el-dialog>
          <el-button
            class="edit-delete-sweets"
            type="text"
...
<script>
import EditDrink from "../Drinks/EditDrink.vue";
import DeleteDrinks from "../Drinks/DeleteDrinks.vue";
export default {
  components: {
    EditDrink,
    DeleteDrinks,
  ...
  computed: {
    src() {
      const filename = this.drink.files?.split(";")[0];
      return filename ? `http://localhost:3000/static/${filename}` : null;
    },
  },
  mounted(){
    this.id=this.drink._id
  },
  methods: {

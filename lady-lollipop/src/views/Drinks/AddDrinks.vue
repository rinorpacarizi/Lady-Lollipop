<template>
  <div>
    ...
    <div>
      <div>
        <form class="form" @submit.prevent="createDrink">
          <DropzoneDrinks
            :drinkId="this.$route.params.id"
            :initialFiles="drink.files"
          />
...
import DropzoneDrinks from "@/components/Dropzones/DropzoneDrinks.vue";
export default {
  name: "AddDrinks",
  components: {
    DropzoneDrinks,
  ...
  methods: {
    async fetchDrink() {
      this.drink = await apiRequest.getDrinks(this.$route.params.id);
    },
    cancel() {
      this.$emit("changeDisplay", false);
    },
    async createDrink() {
      await apiRequest.createDrinks({ ...this.form }).then(() => {
        this.$emit("changeDisplay", false);
      });
    },
  },

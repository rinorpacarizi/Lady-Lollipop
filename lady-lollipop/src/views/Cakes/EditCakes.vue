<template>
  <div style="height: 485px">
    <h1 class="h1-create-edit">Edit Cake</h1>
      ...
      <div>
        <form class="form">
          ...
          <div>
            ...
            <button type="submit" class="submit" @click="editCake">Submit</button>
          </div>
        </form>
      </div>
    </div>
...
export default {
  ...
  data() {
    return {
      form: {
        ...
        stock: this.cakes.stock,
        description: this.cakes.description,
      },
      cake: null,
    };
  },
  ...
  methods: {
    async fetchCake() {
      this.cake = await apiRequest.getCakes(this.$route.params.id);
    },
    cancel() {
      this.$emit("changeDisplay", false);
    },
    async editCake() {
    await apiRequest.editCakes({ ...this.form }).then(()=>{
       this.$emit("changeDisplay", false);
    })
    },
  },
